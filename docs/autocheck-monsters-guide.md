...
3.  Create the following script files and copy the corresponding code from the repository:
    -   **config**: Copy code from `src/sync/config.js`
    -   **checkbox-sync**: Copy code from `src/sync/checkbox-sync.js` 
    -   **flatten-formulas**: Copy code from `src/sync/flatten-formulas.js`
4.  **Configure Sheets**: In the **config** script file, modify the `CONFIG.syncSheetNames` array to include the exact names of all sheets you want to be part of the synchronization.
    ```javascript
    const CONFIG = {
      syncSheetNames: [
        "Collection",
...
The script is composed of multiple files with distinct responsibilities: configuration (`config.js`), live synchronization (`checkbox-sync.js`), and the developer utility (`flatten-formulas.js`).

### File Structure

-   **`config.js`**: Contains the shared `CONFIG` object that defines which sheets participate in synchronization
-   **`checkbox-sync.js`**: Contains the `onEdit` trigger and all functions related to live checkbox synchronization
-   **`flatten-formulas.js`**: Contains the manually-run `flattenCheckboxFormulas` utility function

### Live Synchronization (`onEdit` and supporting functions)

The core logic follows a "source of truth" model, optimized for performance.

1.  **Trigger**: When a user edits a cell in the checkbox column of a configured sheet, the `onEdit` function in `checkbox-sync.js` is triggered.
...
4.  **Sync Target Sheets**: The `syncAllSheets` function then iterates through all other configured sheets. For each target sheet:
    *   It performs a batch read (via `readTargetSheetData`) to get all values, formulas, and data validations.
    *   It compares each row against the "source of truth" map (via `findUpdatesForSheet`).
...
6.  **Apply Changes**: The `applyBatchesToSheet` function loops through the optimized batches and applies all changes for each batch with a single, high-performance `setValues()` call, ensuring maximum speed.

### Formula Flattening (`flattenCheckboxFormulas`)

This developer utility in `flatten-formulas.js` is designed to be both precise and safe.

1.  **Iterate Sheets**: The script loops through all configured sheets except for `Collection` (as defined in the shared `CONFIG` object).
...
All configuration is centralized in the `config.js` file. The main configuration object looks like this:
...
- All listed sheets must have checkboxes in Column A and monster names in Column B
- The script assumes this consistent structure across all configured sheets