...
function flattenCheckboxFormulas() {
  ...
  CONFIG.syncSheetNames.forEach((sheetName) => {
    ...
    for (let row = 0; row < lastRow; row++) {
      const hasFormula = checkboxFormulas[row][0] !== "";
      const validationRule = checkboxValidations[row][0];
      const isCheckbox =
        validationRule?.getCriteriaType() ===
        SpreadsheetApp.DataValidationCriteria.CHECKBOX;
      const monsterName = nameValues[row][0]?.toString();
      ...
      if (isCheckbox && hasFormula && monsterName?.trim()) {
        ...
        dataToWrite.push([checkboxFormulas[row][0]]);
      } else {
        // It's just a static value. Preserve it.
        dataToWrite.push([checkboxValues[row][0]]);
      }
    }
  ...
  console.log(
    `Operation Complete. Replaced ${formulasReplaced} formula(s) across ${sheetsAffected} sheet(s).`
  );
}
 ...
 * @returns {{valid: boolean, sheetName?: string}} Validation result with sheet info.
 */
function validateEditEvent(event) {
 ...
 */
function extractMonsterDataFromSheet(sheet) {
  const CHECKBOX_COL = 1; // Column A
  const NAME_COL = 2; // Column B
...
function onEdit(event) {
  try {
    const validation = validateEditEvent(event);
    if (!validation.valid) {
      return;
    }

    const sheet = event.range.getSheet();
    const sourceDataMap = extractMonsterDataFromSheet(sheet);
    if (!sourceDataMap) {
      return;
    }
    ...
    // --- Synchronization ---
    syncAllSheets(sourceDataMap, validation.sheetName);
  } catch (error) {
